...
async function registerRoutes(app) {
    ...
    app.get("/api/dashboard/metrics", async (req, res) => {
        try {
            ...
            const allOrders = await db_1.db.listOrders();
            ...
            const ordersToday = allOrders.filter((order) => new Date(order.createdAt) >= today).length;
    ...
    app.get("/api/settings/:key", async (req, res) => {
        try {
            ...
            const setting = await settings_service_1.settingsService.getSetting(key);
            if (!setting) {
    ...
    app.put("/api/settings/:key", async (req, res) => {
        try {
            ...
            const updatedSetting = await settings_service_1.settingsService.updateSetting(key, value, category, "user");
    ...
    app.put("/api/settings", async (req, res) => {
        try {
            ...
            const updatedSettings = await settings_service_1.settingsService.updateSettings(settings, "user");
    ...
    app.post("/api/settings/reset", async (req, res) => {
        try {
            const defaultSettings = await settings_service_1.settingsService.resetToDefaults();
    ...
    app.get("/api/settings/export/json", async (req, res) => {
        try {
            const exportData = await settings_service_1.settingsService.exportSettings();
            res.json(exportData);
        }
        catch (error) {
    ...
    app.post("/api/settings/import", async (req, res) => {
        try {
            ...
            const importedSettings = await settings_service_1.settingsService.importSettings(settings, "user");
    ...
    app.get("/api/settings/object/all", async (req, res) => {
        try {
            const settingsObject = await settings_service_1.settingsService.getSettingsObject();
    ...
    settings_service_1.settingsService.initializeDefaults().catch(console.error);
