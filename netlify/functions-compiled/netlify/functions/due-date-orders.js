...
const handler = async (event, context) => {
    ...
    try {
        if (event.httpMethod === 'GET') {
            ...
            switch (type) {
                case 'today':
                    orders = await db_1.db.getTodaysDueOrders();
                ...
                case 'tomorrow':
                    ...
                    orders = (await db_1.db.getOrdersByDueDate(1)).filter(order => {
                    ...
                    break;
                case 'overdue':
                    orders = await db_1.db.getOverdueOrders();
                    title = "Overdue Orders";
                    break;
                case 'upcoming':
                default:
                    const daysAhead = days ? parseInt(days) : 7;
                    orders = await db_1.db.getOrdersByDueDate(daysAhead);
