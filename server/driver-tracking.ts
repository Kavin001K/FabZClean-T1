...
class DriverTrackingService {
  ...
  private driverRoutes: Map<string, RoutePoint[]> = new Map();
...
// Update driver locations every 3 seconds
  ...
  private async simulateDriverForOrder(order: any) {
  ...
  private async createDriverSimulation(order: any): Promise<DriverLocation> {
  ...
  private getWarehouseLocation(): { latitude: number; longitude: number } {
  ...
  private getCustomerLocation(order: any): { latitude: number; longitude: number } {
  ...
  private addRoutePoint(driverId: string, point: RoutePoint) {
  ...
  public getDriverLocation(driverId: string): DriverLocation | null {
    return this.activeDrivers.get(driverId) || null;
  }

  public getDriverRoute(driverId: string): RoutePoint[] {
    return this.driverRoutes.get(driverId) || [];
  }

  public getAllActiveDrivers(): DriverLocation[] {
    return Array.from(this.activeDrivers.values());
  }

  public getDriverForOrder(orderId: string): DriverLocation | null {
  ...
  public async startTrackingForOrder(orderId: string): Promise<DriverLocation | null> {
    try {
  ...
  public stopTrackingForOrder(orderId: string): void {
